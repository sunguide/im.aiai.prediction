...
//登录成功，获取腾讯QQ用户信息
    ...
    //登录成功，获取铂利亚用户信息
    public function boliya($token){
    	$boliya = \ThinkOauth::getInstance('boliya', $token);
    	$data  = $boliya->call('Oauth/getLoggedInUser.shtml');
    	if(!empty($data['uid'])){
    		$userInfo['type'] = 'BOLIYA';
    		$userInfo['name'] = $data['uid'];
    		$userInfo['nick'] = $data['uname'];
    		$userInfo['head'] = "http://tb.himg.baidu.com/sys/portrait/item/{$data['portrait']}";
    		return $userInfo;
    	} else {
    		throw_exception("获取铂利亚用户信息失败：{$data['error_msg']}");
    	}
    }    
